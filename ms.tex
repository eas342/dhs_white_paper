...
There are several variations of readout patterns available to NIRCam with APT.
For exoplanets time series observations, the readout patterns most commonly used will likely be ``RAPID'', meaning every sample is saved up the ramp individually and ``BRIGHT1'' where a single frame is dropped between reads.
These modes give high time resolution and allow one to detect and flag cosmic ray hits without having to discard large amounts of data.
...
For more general discussion of HAWAII-2RG detector readout, we refer to \citet{rauscher2007detectors} and for specific readout patterns we refer to \citet{roberto2009optimalR1} and \citet{roberto2010optimalR2}.

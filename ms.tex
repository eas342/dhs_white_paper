...
Wavelengths & 1.08 - 2.02$\mu$m \footnote{For a field point matched to the F322W2 filter and long wavelength grism.} & 2.44 - 4.02$\mu$m (F322W2)\\
			& 	1.01 - 1.98$\mu$m \footnote{For a field point matched to the F444W filter and long wavelength grism. There is a 0.05$\mu$m gap between detectors that depends on field position.}		& 3.88 - 4.98$\mu$m (F444W) \\
...
\caption{DHS Wavelength coverage for two field positions that will permit grism spectroscopy simultaneously with the F322W2 and F444W filters in the long wavelength arm for the same input source. The V2/V3 axes show the JWST observatory axes and the detectors are labeled for the A channel.}\label{fig:DHSwaveWGrism}
\end{figure}

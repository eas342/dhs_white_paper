...
NIRCam's long-wavelength channel includes grisms that cover 2.4$\mu$m to 5.0$\mu$m with two observations at $R = 1200 - 1550$ and it has already been approved for wide field and single object (time series) slitless spectroscopy.
...
If the region from 2-2.23$\mu$m is highly desirable such as for measuring the CH$_4$ feature shown in Figure \ref{fig:HATp12spec}, the DHS can be used with a F200W filter (1.75 to 2.23$\mu$m half maximum) to partially fill in the gap between the short to long wavelength channels.
...
The size affects the number of DHS that can be included, so the noise in the DHS spectra over 1.1 -- 1.9$\mu$m at the given saturation limit for a 1 hour transit is shown in column 4.}\label{tab:SatSNRsubA}
\end{table}

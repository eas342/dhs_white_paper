...
In this white paper, we discuss a new NIRCam mode (not yet approved or implemented) that can reduce the number of observations needed to cover the 1.0$\mu$m to 5.0$\mu$m wavelength range.
...
NIRCam's long-wavelength channel includes grisms that cover 2.4$\mu$m to 5.0$\mu$m with a resolution of $R = 1200 - 1550$ using two separate configurations.  The long-wavelength grisms have already been approved for wide field and single object (time series) slitless spectroscopy in science operations.
We propose a new mode that will simultaneously measure spectra for science targets in the 1.0$\mu$m to 2.0$\mu$m range using NIRCam's short-wavelength channel.

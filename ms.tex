...
As the exoplanet transit science case will be focused on bright targets, we consider only two of the allowed modes: RAPID (all read frames up the ramp are saved and downlinked) and BRIGHT1
...
We consider a simplified example of a time series of 4 integrations using a standard correlated double sampling (CDS) illustrated in Figure~\ref{fig:readout} and assume an approximate frame time of 10.0 seconds, close to the actual 10.74 seconds full frame read time.
...
If the data is in full frame mode, the cube would have dimensions of 2048 $\times$ 2048 $\times$ 8, which can be thought of 8 image planes.
...
A final reduced time series would consist of flux values at each given wavelength for each of the 4 time coordinates centered on 15 sec, 45 sec, 75 sec and 105 sec relative to the beginning of the observation sequence.%, taking the mid-points between read frames.
...
When the number of groups is 2, the slope can be calculated from the difference between the two reads divided by the frame time.
